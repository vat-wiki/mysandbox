...
- **cookie 鉴权 Path 限 `/proxy`**：`/api/auth/session` 对**每个候选基域**各发一份 Domain cookie（本机 mysandbox.local、远程 tailscale 域名各自种上），被代理页面的 JS 拿 cookie 打不进 `/api/*`。转发上游前剥 cookie 与 `x-sandbox-token`。SameSite=Strict。
- **「都走域名」重定向**：IP/localhost 口径的页面导航 302 到首选基域名（proxy.ts redirect hook）；`/api`、`/ws`、`/proxy` 与非 IP 的其他域名口径豁免（CLI/脚本/探活、tailscale 远程入口不受影响）。
