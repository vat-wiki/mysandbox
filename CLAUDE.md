...
- **创建/应用/迁移都是后台任务**（kind `create`/`apply`/`migrate`，不是 SSE）：`POST /api/services` 快校验（`prepareServiceCreate`：校验/查重/IP 分配，失败回 4xx 内联显示）+ **同步预占名称与 IP**（`serviceNameExists` 查不到「还没建容器」的进行中任务，不锁名会双双通过查重）→ 立即返回 `{jobId}`；写文件/建卷/`compose up` 在 `jobs.ts` 的进程内任务里跑（`runServiceCreate`/`runServiceApply`/`runServiceMigrate`）。任务 = 内存 Map + 环形日志（400 行）+ 终态保留 20 个，**刻意不持久化**（进程重启即丢，重试近乎免费）。路由：`GET /api/services/jobs?tail=`（tail=0 极小 payload）、`GET .../jobs/:id`（全量日志）、`POST .../jobs/:id/cancel`。**compose up 全程可取消**（SIGKILL CLI；半途状态由下一次 up 幂等收敛——声明式底账消解了旧「取消必须落在 rm 之前」的时序枷锁）。配置页 = `GET /api/services/:name/config`（视图含 yaml/path/hash/appliedHash/drift/legacy）+ `POST .../apply`（body yaml；校验写盘在路由同步段 400 回显，up 进 job）+ `POST .../migrate`（旧版服务迁移，见下）。jobs.ts 不运行时 import services.ts（编排以 thunk 传入，防循环依赖）。
...
- **收编外部容器**（`adoptService`/`unadoptService`，routes：`GET /api/services/adoptables`、`POST /api/services/adopt`、`POST /api/services/:name/unadopt`）：把宿主上现成 docker 容器（compose 栈等）纳入服务层——sidecar 写 `adopted: true` 的 meta（preset='adopted'、env 空、volume null）+ `docker network connect mysandbox-lan --ip <池内空闲>`（已在服务网络则复用现 IP 不重连）；容器本体/原网络/env 不动。收编名限定 `/^[a-z0-9][a-z0-9_-]{0,62}$/`（比 NAME_RE 宽出的 `_` 是 compose 惯例；**不含 `.`**——vhost 域名分隔符、不含大写——Host 头被小写化；proxy.ts 的 NAME_RE 是 URL 解析预过滤，已同步放宽）。守卫：收编容器**删除 409**（不是我们的对象），只有「取消收编」（disconnect + 清 meta + 追平 hosts）。事件自愈：`subscribeServiceEvents` 不带 label 过滤（回调给 `managed` 标志），收编容器 `start` 时不在服务网络端点 → `healAdopted` 自动重连（meta.ip 被占则重分配回写）；`destroy` 后 60s 确认无同名重建才清 meta（compose force-recreate 是 destroy→create→start，立刻清会弄丢自愈）。前端：侧栏应用容器分区头 Inbox 与抽屉头部 Inbox 双入口 → `AdoptServiceDialog`（TermSessionsDialog 同款列表模式；侧栏入口由 `ContainerList` 自挂，成功 toast + 即时刷侧栏）；侧栏卡片常驻「收编」Badge、右键菜单无删除/改配置。
