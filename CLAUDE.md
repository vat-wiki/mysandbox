...
docker 引擎移除后 docker 的新角色：**配套服务层**。mysandbox 在宿主 docker 上起单容器服务（postgres/redis/mysql/自定义），挂在与 LXC 互通的 docker 网络（`cfg.services.network`，默认 `dev-lan`），固定 IP + 命名卷；LXC 容器按服务名直连（hosts 自动注入，跨桥互通见上面 LXC 网络段）。
