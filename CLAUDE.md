...
- **前端**：两层分工——侧栏「docker 服务」分区（`ContainerList.vue` 底部环境区：与系统容器同形态的卡片，色条用状态语义色 running=绿/restarting=琥珀/其余灰；分区头 = 图标+计数+聚合状态点，整行点击展开/收起，`mysandbox:svc-cards-open` 记忆，收起时显示摘要文案；卡片 ⋯ 收复制连接命令/打开端口/启停重启，点卡片开抽屉并定位该服务）+ `ServicesPanel` 服务抽屉（右侧滑入 master-detail，裸用 reka 原语自绘而非 ui/dialog 的居中 DialogContent；左栏清单——进行中/失败/取消的创建任务置顶可点开看日志，服务项一行一态，底部刷新+新建；右侧详情——连接命令/env 凭据/元信息/日志常驻 3s 跟刷，任务完成自动选中产出的服务；手机左栏变横滚 chips）。创建走 `ServiceCreateDialog`（预设表单/自定义镜像；shadcn-vue Select（reka-ui portal）——**别用原生 `<select>`**，强制 dark 下 OS 自绘弹层白底违和；提交拿 jobId 即关窗）。完成通知：`lib/serviceJobs.ts` 的 `trackServiceJobs`（模块级 Map 记上次 state，只有「见过 running 落到终态」才 toast——两个轮询器喂它天然去重，首拉不误报）+ 全局 `<Toaster>`（vue-sonner，`ui/sonner/Sonner.vue` 硬编码 dark，无 next-themes）。侧栏服务分区自适应轮询（闲时 15s / 有任务 3s），任务进行中分区头摘要显示「N 个服务任务进行中…」。SSE（`streamOp`）现在只有 base 在用。
