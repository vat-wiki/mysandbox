...
- **目录注册表（agent 接口）**：`compose/<名>/` 目录本身是服务清单——agent/用户直接放一份 compose.yaml（顶层 `name:` = 目录名）+ `docker compose up -d`，面板即出现该服务（listServices 目录 ∪ 容器双源扫描；容器经 compose project label 命中目录名，容器名 ≠ 项目名时视图带 `container` 真名供 docker 直操作）。创建对话框已退役（保留 `POST /api/services` API 与 presets 供 agent 调用）。
...
- **收编外部容器**（`adoptService`，routes：`GET /api/services/adoptables`、`POST /api/services/adopt`，body 带 `takeover`）：两种形态按容器是否有 compose project label 分流——**裸容器（docker run 起家）→ 接管式收编（默认，takeover:true）**：`inspectContainerShape` + `inspectImageDefaults` 复刻「操作者加的形状」（env 减镜像默认、entrypoint/command 仅与镜像默认不同才写、端口/named+匿名卷/healthcheck/restart/原自定义网络原样保留——tl-db 挂 tl-test 网不能丢；host 网络模式写 `network_mode: host` 不进服务网络）生成 compose 底账 → rm 裸容器 → compose up（后台 job，重建中断/可写层丢失由前端确认框挑明）；收编完成后 meta.adopted 清位，按 label+底账双凭证管理（删除/改配置解锁）。**compose 栈容器 → 只读收编**：sidecar 登记 + `docker network connect mysandbox-lan --ip <池内空闲>`（已在服务网络则复用现 IP），本体/原网络/env 不动，删除/配置/迁移一律 409（生命周期归原编排方），只有「取消收编」。收编名限定 `/^[a-z0-9][a-z0-9_-]{0,62}$/`（`_` 是 compose 惯例；不含 `.`——vhost 域名分隔符、不含大写——Host 头小写化）。adoptables 列表 running 优先（Exited 试验残留是头号噪声，前端默认只显示 running）。已收编（只读）的服务升级成接管 = 先取消收编再接管收编（收编对话框只列未收编容器）。事件自愈：`subscribeServiceEvents` 不带 label 过滤（回调给 managed 标志 + composeProject），收编容器 `start` 时不在服务网络端点 → `healAdopted` 自动重连（meta.ip 被占则重分配回写）；`destroy` 后 60s 确认无同名重建才清 meta；**目录注册表服务的启停追平靠 composeProject 命中 compose 目录判定**（事件 Attributes 自带全部 label）。前端：侧栏分区头 Inbox 与抽屉头部 Inbox 双入口 → `AdoptServiceDialog`（接管带 confirmName 确认；成功 toast + 即时刷侧栏）；侧栏卡片「收编」Badge 常驻（只读形态）、右键菜单无删除/改配置。
