...
// AI 工具面板：AI 相关「就位配置」的入口。两种打开方式——
//   全局（缺省）    两个页签：技能中心（库 + 安装规则，持续同步）+ AI 网关（声明式
//                   配置，自动追平）。两者同为「配置声明、自动追平」语义。
//   容器覆盖        props.overrideFor 传入时 = 容器卡片菜单「AI 网关…」的 pull 入口：
//                   只开 AI 网关页签、编辑对象是该容器的覆盖配置（技能是多容器分发
//                   概念，单容器语境无意义，不渲染）。
import { ref } from 'vue'
...
const props = defineProps<{
  // 传入 = 覆盖模式（只显示 AI 网关页签，表单编辑该容器的覆盖配置）。
...
const tab = ref<'hub' | 'gw'>(props.overrideFor ? 'gw' : 'hub')
</script>
    ...
    <DialogContent class="flex max-h-[88vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
      ...
      <Tabs v-model="tab" class="flex min-h-0 flex-1 flex-col gap-0">
        ...
        <div class="scroll-thin min-h-0 flex-1 overflow-y-auto px-5 py-4">
          <TabsContent v-if="!props.overrideFor" value="hub" class="mt-0">
            <SkillsHubTab @unauthorized="emit('unauthorized')" />
          </TabsContent>
          <TabsContent value="gw" class="mt-0">
            <AiGatewayForm
              :container="props.overrideFor ?? undefined"
