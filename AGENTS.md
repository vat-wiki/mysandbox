...
本文件是**跨工具通用**的代理指南（AGENTS.md 标准）：给在本仓库里干活的 AI 编码代理看
（Claude Code / Codex / opencode / pi …）。Claude Code 走同目录的 `CLAUDE.md`，那里只有
一行 `@AGENTS.md` 导入 + Claude 专属补充——**通用约定一律写本文件，别在两处分叉**。
...
- `dockerApi.enabled`（默认关）时容器可经 docker API 桥拿到**宿主 root 级能力**（docker 可挂宿主 /）——桥绑网关 IP、随 mysandbox 进程存活；ufw 信任模型（下条）下 services 网段同样可达 2375（见「docker 服务层」末条）。
- **ufw 信任模型（2026-09-15 起，替代旧逐端口白名单）**：自管网段（LXC/services）= 完全可信的开发工作区，INPUT 各一条全端口 blanket（`firewall.ts` 推导）；需要隔离的只剩外部网段（热点/局域网设备），走 `firewall.allow` 逐端口放行（缺 port = 全端口）。验证防火墙要用**宿主自有端口**（如 xrdp 3389）——docker-proxy 发布的容器端口走 DNAT→FORWARD 不经 INPUT，拿它们测试会得出「本来就通」的假象。
...
- **受管理容器的判定**：在配置的网桥上，或 config 里有 mysandbox 标记（`lxc.environment = MYSANDBOX_MANAGED=true` 纯文本行，可 diff 可手改）。标记不可变，所以易变元数据（displayName、adopted、tags 等）走 **sidecar JSON**（`state.ts`，存 `~/.mysandbox/state.json`，容器名作 key）。adopt 外部容器只写 sidecar，不动容器对象。
...
- **文件面板四足鼎立**：容器 `files.ts`（`/api/files/*`，execRun）、宿主 `hostFiles.ts`（node:fs 直操作，前端以 `__host__` 哨兵切端点）、服务 `serviceFiles.ts`（`'s:'+名`）、SSH `sshFiles.ts`（`'ssh:'+名`）——路由形状/2MB 上限/mtime 乐观锁一比对齐，`api.ts` 的 `filesBase` 哨兵分发让 `FilePanel`/`FileEditorDialog` 零改动换端点。**cd 跟随**：shell 集成发 OSC 7 直达文件面板（回车事件即时检查），1s cwd 轮询降级为兜底；pane 落焦即切换跟随目标，切组按组记忆恢复。
- **peer exec（容器间命令互通）**（`server/peer.ts`）：`mysandbox exec <目标> -- 命令`，目标 `host | c:<容器名> | s:<服务名> | 裸名`（先 LXC 后服务）。宿主 CLI 进程内直调（不需要服务在跑）；容器内 CLI 转发给种子写入的 `~/.config/mysandbox/peer-exec.mjs`（node 客户端，凭据在同目录 `peer.json`，0600）。宿主代为执行：LXC 走 `execRun`（默认 dev）、服务走 `docker exec`（label 边界）、host 直接 spawn。迷你 HTTP 绑网关 IP:7331（dockerApi.ts 同款 EADDRNOTAVAIL 退避、随进程存活、失败非致命）；凭据是独立 `peerToken`（config 生成，与主 token 分离），ufw 由自管网段 blanket 覆盖（信任模型见安全模型节，services 网段同样可调）。每次 exec 记审计日志。
...
全局技能目录是两处独立实体副本（**无软链**）：`~/.agents/skills`（canonical，规则锚它）+ `~/.claude/skills`（孪生跟铺，内容由 skillSync 保持一致，各记一份清单）；分发遇软链落点跳过防穿透。
...
- **宿主 docker 直用（dockerApi，默认关）**：容器内免装 docker、直用宿主 dockerd——`server/dockerApi.ts` 进程内 TCP→docker.sock 透传桥（绑网关 IP:2375，EADDRNOTAVAIL 退避重试，失败非致命，cli.ts 装配）。容器侧配套全随 `dockerApi.enabled` 开关：hosts 尾块注入 `host.docker.internal → 网关`（`currentSvcLines`，域名钉死不随 ipPool 变）；`DOCKER_HOST=tcp://host.docker.internal:2375` 由 engine `attachArgs` 每次 exec 注入 + `scripts/zshrc` 条件导出兜底（tmux 老 server 的 shell 吃不到 exec env）；容器里只要客户端二进制（模板 step docker-cli，静态包只取 `docker`，不装 docker.io——那带 daemon）。安全：docker = 宿主 root 级能力，自管网段全端口互信下 services 网段同样可达（信任模型见安全模型节）；桥随 mysandbox 进程存活。改网段后 hosts 行随 hosts-sync 追平，无需动容器配置。

### AI 工具与技能中心（aiconfig / skillSync / testlens）

- **模型供应商 × Agent 绑定**（`server/aiconfig.ts` 写入器 + `server/aiState.ts` 状态，路由 `/api/ai/*` 在 routes.ts；web `AiWorkspace.vue` 三板块：技能中心/模型供应商/Agent 工具）：三层模型——provider 库（`state.aiProviders`，N 个 OpenAI/Anthropic 兼容网关的凭据与端点）→ 绑定（`state.aiBinding` 全局一份 + `state.aiTargetOverrides` 容器覆盖，引用 provider id 不内联端点）→ 写入器（解析成 `AiApplyPlan` 逐 base 落盘）。**与 batch.ts 的 exec 路线不同，走宿主直写 rootfs**（D1 uid 直通，属主天然正确、容器不必在跑；宿主 home 是同套 writer 的另一 base，与容器同权，旧 `__host__` 覆盖已退役自愈清除）。落点：claude `.claude/settings.json` env 块（单槽，工具自身配置 toolConfig 合并进来、受管绑定键恒赢，一个保存按钮走 `POST /api/ai/claude-config`）、codex `.codex/config.toml` 锚点块 + 顶层 `model_provider`/`model`（单槽重绑即覆盖；**model 必填**——不写 codex 落回内置 gpt-5.x slug、网关没有必 404；model_provider 恒写 = 绑定即启用；toolConfig.codex（approval/sandbox/推理力度/输出详略/上下文窗口等）→ 顶层对应键，保存路径差集回收、sweep 只合并不删，与 claude 顶级键同口径）+ `.codex/.env` env 键值锚点块（codex 0.149+ 启动自读 CODEX_HOME/.env，不依赖 shell——tmux 老窗口也好使；旧版写 `.zshrc` export 的遗留块在写入时按同锚点回收）、opencode `opencode.json`、pi `.pi/agent/models.json`（多槽多 provider 共存）。幂等两类锚点：JSON 只深改本方案的键（保留用户其余配置）；TOML/.env 用 `# >>> <pid> >>>` 标记块整块替换（旧 pid 恒为 myapikey、形状一致 → 存量零迁移）。变体 key = `<pid>-<wire 后缀>`；**opencode 的 anthropic 变体 baseURL 要补 `/v1`**（@ai-sdk/anthropic 只拼 /messages，与 ANTHROPIC_BASE_URL 客户端自拼 /v1/messages 的约定差一级）。保存按工具槽过滤下发——各工具页签独立保存，Claude 页签保存不连带重写 opencode 配置。
- **技能中心**（`server/skillSync.ts`，路由 `/api/skills/*`）：技能库 `STATE_DIR/skills/registry/<名>/` 是唯一技能真相源——**静态快照中心**（入库一律拷贝快照与来源解耦，来源改动不订阅不自动进库，更新走显式动作、先比对内容指纹再动手，源没了库副本冻结保留）。安装规则（sidecar `state.skillsHub.rules`：技能集合+去向+范围，同 to 不允许两条规则）驱动两步同步：库 → 聚合副本 `hub-<hash(to)>/` → 容器去向（按 `<id>.json` 清单记账，「分发过而已消失」的真删，用户自装的其他 skills 不动）。**订阅单向：只订阅库，不订阅来源**——库内容一变安装位置自动跟走；容器 start 事件补发停机期间克隆的项目；规则删除/改去向 → 孤儿按清单收回。落点两处独立实体副本（无软链）见容器契约节。
- **TestLens 种子**（`server/testlens.ts`）：往目标 home 写 `~/.agent-browser/config.json`（cdp 地址）+ `~/.testlens.json`（CLI host），**文件即真相**（view 读实际文件不存 sidecar 状态），install 读-改-写只动 cdp/host 两键；容器内走服务名 `http://testlens:<port>`、本机换算 `localhost`（端口已发布到宿主）。
- **终端活动监测**（`server/activity.ts`）：「agent 干完活/等输入提醒」的服务端底座——周期扫全部运行中容器+宿主 socket 的 tmux 会话，取尾部输出 hash + history_size 判安静；「用户在不在看」刻意由前端判（可见 tab 是 v-show 常驻，tmux session_attached ≠ 在看）。状态不持久化（同 jobs.ts 取舍）。

### Web 代理——「面板外访问容器/服务的端口」

`server/proxy.ts`：容器/服务在自管私网里，外部设备只能摸到宿主——把出口收进面板（同一监听端口、同一 token）。**两条门面一套核心**：vhost 门面（`Host: <name>-<port>.<基域名>`，由 fastify `rewriteUrl` 改写成规范子路径）+ 子路径门面（`/proxy/<c|s>/<name>/<port>/…` 兜底）。详版设计/实测记录在 `docs/web-proxy.md`——碰代理先读它。**端口免带门面**（`https://mysandbox.test/`，不带 `:7321`）= systemd socket 激活透传：`mysandbox-console.socket`（root systemd 持 443 被动 fd，空闲零进程）+ `mysandbox-console.service`（`systemd-socket-proxyd` 字节透传到 config listen；TLS 由 mysandbox 自己终结——user unit 拿不到 `CAP_NET_BIND_SERVICE` 实测 exit 218，这是唯一不给 mysandbox 特权的路），模板在 `scripts/mysandbox-console.*`、install.sh 8.5 段按 config listen 落盘。**泛解析同样自持**：`mysandbox-dns.service`（本机 dnsmasq 127.0.0.1:53，`*.mysandbox.test → 127.0.0.1`，上游实时读 resolved uplink）——`.test` 不可注册、hosts 做不了通配，此前靠 mihomo hosts 应答是环境巧合不是依赖；install.sh 在 127.0.0.1:53 空闲时落盘启用并写 resolved drop-in（`DNS=127.0.0.1`，dnsmasq 上游同读一份文件不成环；已有自配 DNS 的环境跳过）。坑：dnsmasq 的 `--no-resolv` 会把 `--resolv-file` 一起禁掉（no upstream servers → 全部 REFUSED，且 `--log-facility=/dev/null` 下无日志，只能前台跑抓 warning）。要点：
...
- WS 是同路由全声明式 `handler` + `wsHandler` 双挂（wsHandler 类型只在 RouteOptions 上）；基域名 auto = 候选序 LAN sslip → tailscale sslip → `mysandbox.test`（**sslip 打头，2026-09-16 起默认口径**——零配置零申请任何设备可用，代价是 URL 烙 IP；`.test` 兜底：IP 漂移免疫 + 离线可用，宿主侧自持应答，远程探测不通自然跳过），**前端逐个探测择优、全败降级子路径**（lib/proxy.ts probeBase，no-cors 打 `msbprobe.<base>/api/health`）；有自有域名优先自有。
...
- UI 组件库走 **shadcn-vue**（reka-ui 底座），源码拷贝在 `src/components/ui/`。优先 `npx shadcn-vue@latest search` 找现成组件、用语义色（`bg-primary`）不用裸色值。（曾有的 `.claude/skills/shadcn-vue` 技能已删除——2026-09 工作区清空，该目录下别再引用。）
...
每轮开发完成后，请以「UI 走查 / 视觉验收」角色，基于本轮需求或设计稿，对提供的界面截图进行视觉验证。
检查：布局对齐、间距、颜色、字体、图标、组件状态（默认/悬停/聚焦/禁用/加载/错误）、弹窗遮罩层级、文本溢出与空态、响应式、与设计稿一致性、是否引入回归问题。
输出：按 P0/P1/P2 列出问题（位置 + 现象 + 期望 + 修复建议），不确定标“需人工确认”。最后给结论：通过 / 有条件通过 / 不通过，并列出优先修复项。